...
function M.fetch_url(url)
  ...
  local method = config.options.tools.web.method or "simple"

  if method == "dumpling" then
    return M.fetch_url_with_dumpling(url)
  else
    -- Existing simple method implementation
